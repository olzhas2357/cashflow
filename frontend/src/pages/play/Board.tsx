...
export default function Board() {
  ...
  usePlayGameSocket(token, gameId, {
    DICE_ROLLED: (payload) => {
      if (typeof payload.die === 'number') setLastRoll(payload.die)
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6 lg:grid lg:grid-cols-[1fr_320px] lg:items-start lg:gap-6 lg:space-y-0">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            ...
            {lastRoll != null && <> &middot; Last roll: {lastRoll}</>}
          </p>
        </div>
        {canRoll && (
        ...
        {isMyTurn && (game?.turn_status === 'AWAITING_DECISION' || game?.turn_status === 'AWAITING_DEAL_CHOICE') && (
          <p className="text-sm font-medium text-amber-400">Awaiting your decision…</p>
        )}
        {!isMyTurn && game?.status === 'in_progress' && game?.turn_status !== 'AWAITING_MARKET_DECISIONS' && (
      ...
      <div>{me && <FinancialStatement player={me} />}</div>
